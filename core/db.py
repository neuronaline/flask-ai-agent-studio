...
def insert_model_invocation(
    ...
    *,
    ...
    """Insert one row into ``model_invocations``.

    New callers should build a :class:`ModelInvocationLog` and pass it via the
    ``log=`` kwarg; the explicit keyword args remain accepted for backward
    compatibility and act as overrides on top of the dataclass.
    """
    # Build the canonical record from the dataclass when provided; fall back to
    # explicit kwargs otherwise. Explicit kwargs always win so legacy callers
    # continue to override fields they pass.
    if log is None:
        log = ModelInvocationLog()
    log_kwargs = {
        "request_payload": request_payload if request_payload is not None else log.request_payload,
        "response_summary": response_summary if response_summary is not None else log.response_summary,
        "assistant_message_id": assistant_message_id if assistant_message_id is not None else log.assistant_message_id,
        "source_message_id": source_message_id if source_message_id is not None else log.source_message_id,
        "step": step if step is not None else log.step,
        "call_index": call_index if call_index is not None else log.call_index,
        "call_type": call_type if call_type != "agent_step" or log.call_type == "agent_step" else log.call_type,
        "is_retry": is_retry or log.is_retry,
        "retry_reason": retry_reason if retry_reason is not None else log.retry_reason,
        "provider": provider if provider is not None else log.provider,
        "api_model": api_model if api_model is not None else log.api_model,
        "operation": operation if operation is not None else log.operation,
        "prompt_tokens": prompt_tokens if prompt_tokens is not None else log.prompt_tokens,
        "completion_tokens": completion_tokens if completion_tokens is not None else log.completion_tokens,
        "total_tokens": total_tokens if total_tokens is not None else log.total_tokens,
        "estimated_input_tokens": estimated_input_tokens if estimated_input_tokens is not None else log.estimated_input_tokens,
        "prompt_cache_hit_tokens": prompt_cache_hit_tokens if prompt_cache_hit_tokens is not None else log.prompt_cache_hit_tokens,
        "prompt_cache_miss_tokens": prompt_cache_miss_tokens if prompt_cache_miss_tokens is not None else log.prompt_cache_miss_tokens,
        "prompt_cache_write_tokens": prompt_cache_write_tokens if prompt_cache_write_tokens is not None else log.prompt_cache_write_tokens,
        "latency_ms": latency_ms if latency_ms is not None else log.latency_ms,
        "response_status": response_status if response_status is not None else log.response_status,
        "error_type": error_type if error_type is not None else log.error_type,
        "error_message": error_message if error_message is not None else log.error_message,
    }
    log = dataclasses.replace(log, **log_kwargs)

    normalized_prompt_tokens = int(log.prompt_tokens) if log.prompt_tokens is not None else None
    normalized_completion_tokens = int(log.completion_tokens) if log.completion_tokens is not None else None
    normalized_total_tokens = int(log.total_tokens) if log.total_tokens is not None else None
    normalized_estimated_input_tokens = (
        int(log.estimated_input_tokens) if log.estimated_input_tokens is not None else None
    )
    normalized_prompt_cache_hit_tokens = (
        int(log.prompt_cache_hit_tokens) if log.prompt_cache_hit_tokens is not None else None
    )
    normalized_prompt_cache_miss_tokens = (
        int(log.prompt_cache_miss_tokens) if log.prompt_cache_miss_tokens is not None else None
    )
    normalized_prompt_cache_write_tokens = (
        int(log.prompt_cache_write_tokens) if log.prompt_cache_write_tokens is not None else None
    )
    normalized_latency_ms = int(log.latency_ms) if log.latency_ms is not None else None
    if (
    ...
    raw_payload = _serialize_json_value(log.request_payload)
    ...
    cursor = conn.execute(
        ...
        (
            int(conversation_id),
            _coerce_positive_int(log.assistant_message_id),
            _coerce_positive_int(log.source_message_id),
            max(0, int(log.step or 0)),
            max(0, int(log.call_index or 0)),
            str(log.call_type or "agent_step").strip() or "agent_step",
            1 if log.is_retry else 0,
            str(log.retry_reason or "").strip() or None,
            str(log.provider or "").strip(),
            str(log.api_model or "").strip(),
            raw_payload,
            _serialize_json_value(log.response_summary if log.response_summary is not None else {}),
            str(log.operation or "").strip() or None,
            ...
            str(log.response_status or "").strip() or None,
            str(log.error_type or "").strip() or None,
            str(log.error_message or "").strip() or None,
            payload_bytes,
            payload_hash,
        ),
    )
...
# ---------------------------------------------------------------------------
# Re-exports for the per-field normalization helpers.
#
# The 24 ``_normalize_*`` helpers extracted into ``core/normalize.py`` are
# kept available under ``core.db`` so internal callers (and any external
# consumer importing ``from core.db import _normalize_user_profile_value``)
# continue to work unchanged. New code should prefer
# ``from core.normalize import _normalize_*`` directly.
# ---------------------------------------------------------------------------
from core.normalize import (  # noqa: E402,F401  (intentional re-export block)
    _normalize_app_setting_value,  # noqa: F401
    _normalize_clarification_dependency_payload,  # noqa: F401
    _normalize_clarification_option_payload,  # noqa: F401
    _normalize_clarification_question_payload,  # noqa: F401
    _normalize_conversation_memory_entry_type,  # noqa: F401
    _normalize_conversation_memory_key,  # noqa: F401
    _normalize_conversation_memory_snapshot_entries,  # noqa: F401
    _normalize_conversation_memory_value,  # noqa: F401
    _normalize_initial_image_analysis,  # noqa: F401
    _normalize_message_attachment,  # noqa: F401
    _normalize_message_tool_calls,  # noqa: F401
    _normalize_message_tool_result,  # noqa: F401
    _normalize_message_tool_trace_entry,  # noqa: F401
    _normalize_message_usage,  # noqa: F401
    _normalize_message_usage_call,  # noqa: F401
    _normalize_persona_behavior,  # noqa: F401
    _normalize_persona_memory_key,  # noqa: F401
    _normalize_persona_memory_value,  # noqa: F401
    _normalize_state_mutation_operation,  # noqa: F401
    _normalize_state_mutation_source_message_id,  # noqa: F401
    _normalize_state_mutation_target_key,  # noqa: F401
    _normalize_state_mutation_target_kind,  # noqa: F401
    _normalize_usage_breakdown,  # noqa: F401
    _normalize_user_profile_value,  # noqa: F401
    normalize_persona_name,  # noqa: F401
)
